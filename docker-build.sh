...
if [ "$#" -eq 1 ] && [ "$1" = "bash" ]; then
    ...
    exec docker run -it  --rm -v "$(pwd):/src" -w /src -e TARGET -e KERNEL_TEST_GROUPS --entrypoint bash $DOCKER_IMG_NAME
else
    # run build - use our own UID/GID to create files with correct owner
    exec docker run -it --user "$DOCKER_USER" --rm -v "$(pwd):/src:delegated" -w /src "${TMPFS_OVERLAY[@]}" -e TARGET -e KERNEL_TEST_GROUPS $DOCKER_IMG_NAME "$@"
fi
